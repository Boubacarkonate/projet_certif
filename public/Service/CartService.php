...
class CartService {
    private $session;
    private $formuleRecruteurRepository;


    public function __construct(
        RequestStack $requestStack,
        FormuleRecruteurRepository $formuleRecruteurRepository)
    {
        $this->session=$requestStack;
        $this->formuleRecruteurRepository=$formuleRecruteurRepository;
    }
 ...
 public function show() {
        ...
        // clé de 7 sa valeur est la quantité
         foreach ($panier as $key => $value  ){
            $selection_encours= $this->formuleRecruteurRepository->find($key);

            $panier_complet[]=[
                'formule'=> $selection_encours ,
                'quantite'=>$value,
                'total'=>($selection_encours->getPrix()*$value),
                ];
    ...
    public function getTotalAll(){
        ...
        foreach ($panier as $key => $value  ){
            ...
            $total=  $total + ($this->formuleRecruteurRepository->find($key)->getPrix()*$value);
